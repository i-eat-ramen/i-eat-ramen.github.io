...
function validateReason() {
  ...
  if (document.getElementById("reason-3").checked) {
    ...
    !document.getElementById("reason-1").checked &&
    !document.getElementById("reason-2").checked
  ) {
    valid = false;
...
function validateText(id) {
  ...
  //email doesn't need because it automatically self validates
    ...
    if (valid === false) {
      errorfield.appendChild(
        document.createTextNode(`ERROR: ${id} field must be composed of only alphabetic characters`)
      );
      errorfield.appendChild(document.createElement("br"));
    }
  }
